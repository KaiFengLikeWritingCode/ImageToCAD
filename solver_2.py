...
def constraints(vars, geom_objects, constraint_list):
    ...
    for c in constraint_list:
        ...
        elif type_ == 'endpoint_tangent_line_arc':
            # 约束：在**指定端点**相切，并控制方向（同向/反向）
            # 参数：
            #   line: 线对象索引
            #   arc:  弧对象索引
            #   line_end: 0=线起点, 1=线终点（用哪一端作为连接端）
            #   arc_end:  0=弧起点theta1, 1=弧终点theta2（用哪一端作为连接端）
            #   same:     True=同向切（dot=+1），False=反向切（dot=-1）
            line = geom_objects[c['line']]
            arc = geom_objects[c['arc']]

            # 线的方向向量（从start->end）
            vx = line.params[2] - line.params[0]
            vy = line.params[3] - line.params[1]
            nv = np.hypot(vx, vy)
            if nv < 1e-12:
                # 零长度线，给个温和惩罚避免奇异
                eqs.append(1.0)
                ...
                # 弧端点的切向向量（逆时针：t切向=(-sin t, cos t)）
                t = arc.params[4] if c.get('arc_end', 0) == 1 else arc.params[3]
                tx, ty = -np.sin(t), np.cos(t)

                # 1) 切向平行（叉积=0）
                eqs.append(lvx * ty - lvy * tx)

                # 2) 方向一致性（点积=±1）
                desired = 1.0 if c.get('same', True) else -1.0
                eqs.append(lvx * tx + lvy * ty - desired)
