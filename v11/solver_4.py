...
def constraints(vars, geom_objects, constraint_list):
    ...
    for c in constraint_list:
        ...
        # ---------- 限制弧扫角：|t2 - t1| <= max_rad ----------
            ...
            """
            让弧（按 anchor='start'/'end' 的切线方向）相对于某条线保持在左/右侧的走廊内
            keys:
              line, arc
              anchor: 'start' 或 'end'（用哪个端点的切线方向来定义线的左右）
              side: 'left' / 'right'
              samples: 采样点个数（1 或 3）
              margin: 允许的最小距离（>=0）
              weight: 权重
            ...
            ts = [0.5 * (t1 + t2)] if samples <= 1 else [t1, 0.5 * (t1 + t2), t2]
            ...
            for t in ts:
                ...
                s = (px - x0) * nx + (py - y0) * ny
                if side == 'left':
                    eqs.append(w * soft_hinge(margin - s))
                elif side == 'right':
                    eqs.append(w * soft_hinge(s + margin))
                else:
                    raise ValueError("arc_side_of_line.side must be 'left' or 'right'")
        elif type_ == 'line_endpoint_x_order':
            """
            约束线段两端点谁在右边（x 更大）
            用法示例：
              # 端点1 (x2,y2) 在右边（x2 >= x1 + margin）
              {'type':'line_endpoint_x_order','line':idx,'right_index':1,'margin':0.0}

              # 端点0 (x1,y1) 在右边（x1 >= x2 + margin）
              {'type':'line_endpoint_x_order','line':idx,'right_index':0,'margin':0.0}
            说明：
              - right_index: 取 0 或 1，表示哪个端点必须在右侧
              - margin: 可选的最小水平间距（>=0），默认 0
            """
            L = geom_objects[c['line']]
            (x0, y0), (x1, y1) = L.points()
            right_idx = int(c.get('right_index', 1))
            margin = float(c.get('margin', 0.0))

            if right_idx == 1:
                # 要求 x1 >= x0 + margin
                eqs.append(soft_hinge((x0 + margin) - x1))
            elif right_idx == 0:
                # 要求 x0 >= x1 + margin
                eqs.append(soft_hinge((x1 + margin) - x0))
            else:
                raise ValueError("line_endpoint_x_order.right_index must be 0 or 1")
        elif type_ == 'line_through_point':
            """
            约束：线段所在的无界直线必须经过指定点 P

            用法一（直接给坐标）：
              {'type':'line_through_point', 'line': L_idx, 'px': 120.0, 'py': 35.0}

            用法二（引用其他几何对象的端点）：
              {'type':'line_through_point', 'line': L_idx, 'p_obj': k, 'which': 0}  # which=0/1

            说明：
              - 这是“经过点”的强等式约束：点到该直线的有符号距离应为 0
              - 只约束无界直线，不要求点在线段的有限长度范围内
            """
            L = geom_objects[c['line']]
            (x0, y0), (x1, y1) = L.points()
            ...
            # 取目标点 P(px,py)
            if 'p_obj' in c:
                Pobj = geom_objects[c['p_obj']]
                which = int(c.get('which', 0))
                px, py = Pobj.points()[which]
            else:
                px = float(c['px'])
                py = float(c['py'])

            # 直线一般式 Ax + By + C = 0  （由两点式转换）
            A = y0 - y1
            B = x1 - x0
            C = x0 * y1 - x1 * y0

            den = np.hypot(A, B)  # = sqrt(A^2+B^2)

            if den < 1e-12:
                # 线段退化，给温和惩罚，推动端点分离（避免数值不稳定）
                eqs.append(1.0)
            else:
                # 点到直线的有符号距离 / |(A,B)| → 目标为 0
                eqs.append((A * px + B * py + C) / den)
        elif type_ == 'arc_vertical_gap_at_x':
            """
            约束：在给定 x0 处，两段弧的纵坐标差等于 value
            用法：
              {'type':'arc_vertical_gap_at_x','Aarc':i,'Barc':j,'x':x0,'value':25.0}

            可选项：
              'penalty_weight': 对不相交/越界时的软惩罚权重（默认 1.0）
            """
            ...
            def in_arc_interval(t, t1, t2):
                # 判断 t 是否落在按 t1->t2 的弧区间内（包含端点），考虑环绕
                s = sweep_len(t2, t1)  # 区间长度 ∈ [0, 2π)
                w = (t - t1) % (2 * np.pi)  # t 相对 t1 的正向偏移
                return w <= s + 1e-12

            def angdist_to_interval(t, t1, t2):
                # t 到区间 [t1->t2] 的最小角距离（0 表示在区间内）
                s = sweep_len(t2, t1)
                w = (t - t1) % (2 * np.pi)
                if w <= s:  # 落在区间内
                    return 0.0
                # 到两端点的最小角距离
                d1 = abs(((t - t1 + np.pi) % (2 * np.pi)) - np.pi)
                d2 = abs(((t - t2 + np.pi) % (2 * np.pi)) - np.pi)
                return min(d1, d2)

            def y_at_x_on_arc(arc, x0):
                cx, cy, r, t1, t2 = arc.params
                u = (x0 - cx) / (r + 1e-12)

                # 圆与竖线是否有交：|u|<=1
                # 数值上先裁剪到 [-1,1] 以得到候选角；同时记录越界量作惩罚
                out_of_range = max(0.0, abs(u) - 1.0)
                u_clamped = np.clip(u, -1.0, 1.0)

                # 两个主值解：±acos(u)
                base = np.arccos(u_clamped)
                cand = np.array([base, -base])  # 注意 sin(-θ)=-sinθ，cos(-θ)=cosθ

                # 从候选里选落在弧区间内的；若都不在，选离区间最近的那个
                chosen = None
                min_gap = 1e9
                for t in cand:
                    if in_arc_interval(t, t1, t2):
                        chosen = t
                        break
                    else:
                        gap = angdist_to_interval(t, t1, t2)
                        if gap < min_gap:
                            min_gap = gap
                            chosen = t

                y = cy + r * np.sin(chosen)
                return y, out_of_range, min_gap

            Aarc = geom_objects[c['Aarc']]
            Barc = geom_objects[c['Barc']]
            x0 = float(c['x'])
            target = float(c['value'])
            wpen = float(c.get('penalty_weight', 0.5))

            yA, missA, gapA = y_at_x_on_arc(Aarc, x0)
            yB, missB, gapB = y_at_x_on_arc(Barc, x0)

            # 1) 主要等式： (yA - yB) - target = 0
            eqs.append((yA - yB) - target)
            # eqs.append(wpen * soft_hinge(missA))  # |x0-cx|>r 的越界量
            # eqs.append(wpen * soft_hinge(missB))
            # eqs.append(wpen * soft_hinge(gapA))  # 角不在 [t1->t2] 的角距
            # eqs.append(wpen * soft_hinge(gapB))
