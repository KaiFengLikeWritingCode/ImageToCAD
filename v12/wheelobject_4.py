...
# 7 右上水平短边
L_right_top_h = Line(x1=None, y1=68+135, x2=420, y2=68+135)
geom_objects.append(L_right_top_h)

# 8 右下水平短边
L_right_bottom_h = Line(x1=None, y1=68.0, x2=420, y2=68.0)
...
# 13：
l_top_hu3 = Arc(cx=380, cy=-42, r=195.0, theta1=None, theta2=None)
...
# 17 ：
l_bottom_hu3 = Arc(cx=380, cy=-42, r=176.0, theta1=None, theta2=None)
...
'''
# 21： 右侧基线
L_right_vert = Line(x1=420.0, y1=68, x2=420.0, y2=68.0 + 135)   # 我把左右两端都先固定为便于约束链
...
constraint_list += [
    ...
    {'type':'tangent_at_arc_start_to_line', 'line':3, 'arc':11, 'side':'right'},
    ...
    {'type':'tangent_at_arc_end_to_line', 'line':4, 'arc':15, 'side':'right'},

    # b14.end 与 a5.start 相连 → 在 b14.end 处与 a5 相切
    {'type':'tangent_at_arc_end_to_line',   'line':5, 'arc':14, 'side':'left'},
    ...
    {'type':'tangent_at_arc_start_to_line',   'line':6, 'arc':18, 'side':'left'},
...
# 7、8 等长
constraint_list.append({'type':'point_distance_x', 'p1':7, 'which1':0, 'p2':8, 'which2':0, 'value':0.0})
constraint_list.append({'type':'point_distance_x', 'p1':1, 'which1':1, 'p2':2, 'which2':1, 'value':0.0})
# 11、15的距离
constraint_list.append({
    'type':'arc_vertical_gap_at_x',
    'Aarc':11,        # 上方那段弧
    'Barc':15,        # 下方那段弧
    'x':132.0+55,         # 竖线位置（按你的图纸给真实值）
    'value':25.0,     # 纵坐标差
    'penalty_weight':1.0  # 可调：越大越强推向可行交点
})
...
constraint_list += [
    {'type':'line_endpoint_x_order','line':1,'right_index':1},
    {'type':'line_endpoint_x_order','line':2,'right_index':1},
    {'type':'line_endpoint_x_order','line':3,'right_index':1},
    {'type':'line_endpoint_x_order','line':4,'right_index':1},
    {'type':'line_endpoint_x_order','line':5,'right_index':1},
    {'type':'line_endpoint_x_order','line':6,'right_index':1},
    {'type':'line_endpoint_x_order','line':7,'right_index':1},
    {'type':'line_endpoint_x_order','line':8,'right_index':1},
]
constraint_list += [
    {'type': 'line_through_point', 'line': 3, 'px': 132.0, 'py': 178.0},
    {'type': 'line_through_point', 'line': 4, 'px': 132.0, 'py': 0.0},
    {'type': 'line_through_point', 'line': 5, 'px': 320.0, 'py': 68.0+135},
    {'type': 'line_through_point', 'line': 6, 'px': 320.0, 'py': 68.0},
...
for idx, seed in [(5, (365.0, 68+110, 370.0, 135.0+68)),   # a5  右上斜线
                  (6, (365.0,  80.0, 370.0,  68.0))]:  # a6  右下斜线
    L = geom_objects[idx]
    for j, v in enumerate(seed):
        if np.isnan(L.params[j]): L.params[j] = v
